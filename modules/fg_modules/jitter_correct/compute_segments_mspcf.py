...
def filter_adata(signals_adata):
    # remove bad cells
    # MA: we don't need to remove these
    # signals_adata = signals_adata[
    #    (signals_adata.obs['is_normal'] == False) &
    #    (signals_adata.obs['is_aberrant_normal_cell'] == False) &
    #    (signals_adata.obs['is_doublet'] == 'No') &
    #    (signals_adata.obs['is_s_phase_thresholds'] == False)
    #]
    
    # remove non_signals columns
    signals_adata = signals_adata[:, 
                                  ~np.all(
                                      np.isnan(signals_adata.layers['Min']),
                                      axis = 0)
                                 ].copy()

    # sort columns in genomic order
    signals_adata.var['iindex'] = np.arange(len(signals_adata.var))
    sorted_var_order = signals_adata.var.sort_values(by = ['chr', 'start']).iindex
    signals_adata = signals_adata[:, sorted_var_order]
    
    return signals_adata
...
def preprocess_data(adata, hg19_blacklist_path, n_bins_trim_telomere = 2, centromere_buffer_size = 2.5e6,
                  skipY = True):
    """
    ...
    """
    chr2centro = json.load(open(hg19_blacklist_path, 'r'))
    ...
    for i in range(len(my_thresholds) - 1):
        ...
        if skipY and str(ch).endswith("Y"):
            continue
    ...
    for ch, myX in X_by_chr.items():
        ...
        if ch == 'Y':
            continue
        ch_var = adata.var[adata.var['chr'] == ch]
        if ch_var.shape[0] ==0:
            assert False, "No reads found for chr..."
        ...
        # centromere_bounds is keyed by chromosome (not by cell): the invalid
        # region is derived from adata.var, which is the same bin layout for
        # every cell, so the bounds used are identical across all cells
        centromere_bounds[ch] = {
            'chr': ch,
            'centromere_start': start_invalid_region,
            'centromere_end': end_invalid_region,
        }
    ...
    outputs['centromere_bounds'] = centromere_bounds
...
def find_segments_mspcf(
        ...
        gamma=0.12,
        trim_telomere=2,
        centromere_buffer=2.5,
        include_y=False,
    ):
    # MA: using read_h5ad instead of read
    signals_adata = ad.read_h5ad(signals_adata)
    signals_adata.var['chr'] = signals_adata.var['chr'].astype(str)
    signals_adata.var['start'] = signals_adata.var['start'].astype(int)
    signals_adata.var['end'] = signals_adata.var['end'].astype(int)

    signals_adata = filter_adata(signals_adata)
    cells = set([a.strip() for a in open(cell_list_path).readlines()])
    assert np.all([c in signals_adata.obs.brief_cell_id.values for c in cells]), "Some cells in cell list not found in adata"
    signals_adata = signals_adata[signals_adata.obs.brief_cell_id.isin(cells), :].copy()

    inputs = preprocess_data(signals_adata, 
                             hg19_blacklist_path=hg19_blacklist_path,
                             n_bins_trim_telomere=trim_telomere,
                             centromere_buffer_size=centromere_buffer * 1e6,
                             skipY=not include_y)

    sorted_ch_keys = sorted(inputs['Xnorm_by_arm'].keys(), 
                            key = lambda x: (int(x[:-1]) if x[:-1] != 'X' else 23) + (0.5 if x[-1] == 'q' else 0))

    assert cores >= 1, "Number of cores must be positive"
    if cores == 1:
        results = map(run_mspcf_arm_gamma, 
                      [(inputs['Xnorm_by_arm'][ch], inputs['baf_by_arm'][ch], gamma) for ch in sorted_ch_keys])
    else:
        with Pool(cores) as p:
            results = p.map(run_mspcf_arm_gamma, 
                            [(inputs['Xnorm_by_arm'][ch], 
                              inputs['baf_by_arm'][ch], gamma) for ch in sorted_ch_keys])
    results = {k:v for k,v in zip(sorted_ch_keys, results)}

    segments = bps2segments(inputs, results, signals_adata)

    cn_table = unstack_adata(signals_adata, layers = ['state', 'Min', 'Maj'])
    resegmented = resegment(cn_table, segments, cn_cols = ['state', 'Min', 'Maj'],
                           allow_bins_dropped = True)
    
    # add "state_phase" categorization required for sitka plotting 
    # note: these categories are overlapping so the order of these statements matters!
    resegmented['state_phase'] = np.zeros(len(resegmented), dtype = 'object')
    resegmented.loc[resegmented.Maj > resegmented.Min, 'state_phase'] = 'A-Gained'
    resegmented.loc[resegmented.Maj < resegmented.Min, 'state_phase'] = 'B-Gained'
    resegmented.loc[resegmented.Min == 0, 'state_phase'] = 'A-Hom'
    resegmented.loc[resegmented.Maj == 0, 'state_phase'] = 'B-Hom'
    resegmented.loc[resegmented.Maj == resegmented.Min, 'state_phase'] = 'Balanced'

    # reorder columns and drop segment_idx
    resegmented = resegmented[['cell_id', 'chr', 'start', 'end', 'state', 'Min', 'Maj', 'state_phase']]
    # MA: rename some of the columns so medicc2 can use this input
    resegmented = resegmented.rename(columns={'cell_id': 'sample_id', 'chr': 'chrom', 'Maj': 'cn_a', 'Min': 'cn_b'})

    #MA: adding sep='\t'
    resegmented.to_csv(output_file, sep='\t', index = False)

    # centromere bounds are identical for every cell (see preprocess_data), so
    # write out a single row per chromosome rather than per cell
    centromere_bounds_df = pd.DataFrame(inputs['centromere_bounds'].values())
    centromere_bounds_output = os.path.join(os.path.dirname(output_file) or '.', 'centromere_bounds.tsv')
    centromere_bounds_df.to_csv(centromere_bounds_output, sep='\t', index = False)

    return signals_adata



def color_tree_wgd(events, tree):
    """
    Parameters:
        events: pandas dataframe of MEDICC2 _events_overlap.tsv table
        tree: Bio.Phylo tree from MEDICC2
    """
    ...
    cells_affected = {}
    nodes_affected = {}
    for _, r in events[events.type == 'wgd'].iterrows():
        wgd_branch = r.sample_id
        wgd_clade = [a for a in tree.find_clades(wgd_branch)]
        assert len(wgd_clade) == 1
        wgd_clade = wgd_clade[0]

        cells_affected[wgd_clade.name] = [a.name for a in wgd_clade.get_terminals()]
        nodes_affected[wgd_clade.name] = [a.name for a in wgd_clade.get_terminals()] + [a.name for a in wgd_clade.get_nonterminals()]
    ...
    return n_wgds, cells_affected, nodes_affected
...
@click.argument('cell_list_path')
@click.argument('output_path')
@click.option('--hg19_blacklist_path', default='helper/hg19.chr2centro.json', type=click.Path(), help='Path to hg19 blacklist file')
@click.option('cores', '--cores', default=1, type=int)
@click.option('gamma', '--gamma', default=0.12, type=float, help = 'Penalty for adding a segment')
@click.option('trim_telomere', '--trim_telomere', default=2, type=int, help = 'Number of bins to trim from telomeres')
@click.option('centromere_buffer', '--centromere_buffer', default=2.5, type=float, help = 'Number of MB from centromeres to ignore')
@click.option('include_y', '--include_y', default=False, type=bool, help = 'Whether to include chrY in segmentation')
...
def run_mspcf(
    ...
    # this function loads and filters the anndata, runs mspcf, and writes out segments
    adata = find_segments_mspcf(signals_adata_path, cell_list_path, output_path, 
                                hg19_blacklist_path=hg19_blacklist_path,
                                cores=cores, gamma=gamma, 
                                trim_telomere=trim_telomere, 
